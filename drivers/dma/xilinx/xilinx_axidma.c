...
struct xilinx_dma_desc_sw {
	...
	struct xilinx_callback_param callback_param;
...
static void xilinx_chan_desc_cleanup(struct xilinx_dma_chan *chan)
{
	...
	while (!list_empty(&chan->active_list)) {
		...
		hw = &desc->hw;
		if (hw->control & XILINX_DMA_BD_SOP)
			total_requested = 0;
		if (hw->status & XILINX_DMA_BD_SOP)
			total_transferred = 0;
		total_requested += hw->control & XILINX_DMA_MAX_TRANS_LEN;
		total_transferred += hw->status & XILINX_DMA_MAX_TRANS_LEN;
		...
		if (callback && callback_param) {
			callback_param->requested_len = total_requested;
			callback_param->transferred_len = total_transferred;
			spin_unlock_irqrestore(&chan->lock, flags);
...
static void xilinx_dma_update_completed_cookie(struct xilinx_dma_chan *chan)
{
	...
	list_for_each_entry(desc, &chan->active_list, node) {
		...
		if (chan->has_sg) {
			...
			u32 transfered = dma_read(chan, XILINX_DMA_BTT_OFFSET) &
					 XILINX_DMA_MAX_TRANS_LEN;
			hw->status = XILINX_DMA_BD_SOP | XILINX_DMA_BD_EOP | transfered;
			chan->completed_cookie = desc->async_tx.cookie;
		}
	}
}
